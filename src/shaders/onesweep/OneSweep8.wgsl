//****************************************************************************
// GPUSorting
// OneSweep - WaveSize 8 Variant (BlockDim 128)
//****************************************************************************
...
@group(0) @binding(1) var<storage, read_write> bump: array<atomic<u32>>;
...
@group(0) @binding(7) var<storage, read_write> pass_hist: array<atomic<u32>>;
@group(0) @binding(8) var<storage, read_write> status: array<u32>;

// --- SIMD8 CONFIGURATION ---
const BLOCK_DIM = 128u;
const MIN_SUBGROUP_SIZE = 8u;
const REDUCE_BLOCK_DIM = 64u; // Reduced to keep shared mem size down
// ---------------------------
...
fn global_hist(
    ...
    if (lane_count < MIN_SUBGROUP_SIZE || (REDUCE_BLOCK_DIM % lane_count) != 0u) {
        if (threadid.x == 0u) { status[STATUS_ERR_GLOBAL_HIST] = 0xDEAD0001u; }
        return;
    }
...
fn onesweep_scan(
        ...
        for(var j = lane_count; j <= aligned_size; j <<= lane_log){
            ...
            if(j != lane_count){
                ...
                if ((i1 & (j - 1u)) >= rshift){
                    ...
                    if(pred1 && ((i1 + 1u) & (rshift - 1u)) != 0u){ wg_scan[i1] += t1; }
                }
            } else { offset0 += 1u; }
            offset1 += lane_log;
        }
    ...
    var spine_prefix = 0u;
    if (sid > 0u) { spine_prefix = wg_scan[sid - 1u]; }
...
fn WLMS(key: u32, shift: u32, laneid: u32, lane_count: u32, lane_mask_lt: u32, s_offset: u32, key_valid: bool) -> u32 {
    ...
    if (!key_valid) { eq_mask = 0u; }
    var subgroup_mask = 0xffffffffu;
    if (lane_count != 32u) { subgroup_mask = (1u << lane_count) - 1u; }
    ...
    workgroupBarrier();
    var broadcast = 0u;
    if (eq_mask != 0u) { broadcast = unsafeSubgroupShuffle(pre_inc, highest_rank_peer); }
    out += broadcast;
...
fn onesweep_pass(
    ...
    if (subgroup_hist_size > WARP_HIST_CAPACITY) {
        if (threadid.x == 0u) { status[STATUS_ERR_PASS] = 0xDEAD0004u; }
        return;
    }
    ...
    if (threadid.x == 0u) { wg_broadcast = atomicAdd(&bump[shift >> 3u], 1u); }
    ...
    if (threadid.x < lane_count) {
        ...
        if (pred) { wg_localHist[threadid.x * lane_count] = t; }
    }
    workgroupBarrier();
