//****************************************************************************
// GPUSorting
// Device Radix Sort - SIMD8 Variant (BlockDim 128)
//****************************************************************************
...
@group(0) @binding(0) var<uniform> info : InfoStruct; 
@group(0) @binding(1) var<storage, read_write> bump: array<u32>;
@group(0) @binding(2) var<storage, read_write> sort: array<u32>;
@group(0) @binding(3) var<storage, read_write> alt: array<u32>;
@group(0) @binding(4) var<storage, read_write> payload: array<u32>;
@group(0) @binding(5) var<storage, read_write> alt_payload: array<u32>;
@group(0) @binding(6) var<storage, read_write> hist: array<atomic<u32>>;
@group(0) @binding(7) var<storage, read_write> pass_hist: array<u32>;
@group(0) @binding(8) var<storage, read_write> status: array<u32>;

// --- SIMD8 CONFIGURATION ---
const BLOCK_DIM = 128u;        // Reduced from 256
const MIN_SUBGROUP_SIZE = 8u;  // Reduced from 16
// ---------------------------
...
const REDUCE_HIST_SIZE = REDUCE_BLOCK_DIM / MIN_SUBGROUP_SIZE * RADIX;
...
fn reduce_hist(
    ...
    if (lane_count < MIN_SUBGROUP_SIZE || (REDUCE_BLOCK_DIM % lane_count) != 0u) {
        if (threadid.x == 0u) { status[STATUS_ERR_REDUCE] = 0xDEAD0001u; }
        return;
    }
    ...
    if (threadid.x < lane_count) {
        ...
        if (pred) { atomicStore(&wg_globalHist[threadid.x * lane_count], t); }
    }
    workgroupBarrier();
    
    for (var i = threadid.x; i < RADIX; i += REDUCE_BLOCK_DIM) {
        atomicAdd(&hist[i + radix_shift * 32u],
...
fn scan(
    ...
    if (lane_count < MIN_SUBGROUP_SIZE || (BLOCK_DIM % lane_count) != 0u) {
        if (threadid.x == 0u) { status[STATUS_ERR_SCAN] = 0xDEAD0002u; }
        return;
    }
    if (wgid.x == 0u && threadid.x == 0u) {
        let pass_idx = info.shift / RADIX_LOG;
    ...
    for(var dev_offset = 0u; dev_offset < aligned_size; dev_offset += SCAN_PART_SIZE){
        {
            ...
            for(var k = 0u; k < SCAN_SPT; k += 1u){
                if(i < info.thread_blocks){ t_scan[k] = pass_hist[i + radix_offset]; }
                i += lane_count;
            }
        }
        var prev = 0u;
        for(var k = 0u; k < SCAN_SPT; k += 1u){
            ...
            prev = unsafeSubgroupShuffle(t_scan[k], lane_count - 1u);
        }
        if(laneid == lane_count - 1u){ wg_reduce[sid] = prev; }
        workgroupBarrier();
            ...
            for(var j = lane_count; j <= local_aligned_size; j <<= lane_log){
                ...
                if(j != lane_count){
                    ...
                    if ((i1 & (j - 1u)) >= rshift){
                        ...
                        if(pred1 && ((i1 + 1u) & (rshift - 1u)) != 0u){ wg_reduce[i1] += t1; }
                    }
                } else { offset0 += 1u; }
                offset1 += lane_log;
            }
...

        {
            ...
            for(var k = 0u; k < SCAN_SPT; k += 1u){
                if(i < info.thread_blocks){ pass_hist[i + radix_offset] = t_scan[k] + prev; }
                i += lane_count;
            }
        }
...
@diagnostic(off, subgroup_uniformity)
fn WLMS(key: u32, shift: u32, laneid: u32, lane_count: u32, _unused: u32, s_offset: u32) -> u32 {
...
fn dvr_pass(
    ...
    if (warp_hists_size > WARP_HIST_CAPACITY) {
        if (threadid.x == 0u) { status[STATUS_ERR_DVR] = 0xDEAD0004u; }
        return;
    }
    if (wgid.x == 0u && threadid.x == 0u) {
        let pass_idx = info.shift / RADIX_LOG;
    ...
    if (threadid.x < lane_count) {
        ...
        if (pred) { atomicStore(&wg_warpHist[threadid.x * lane_count], t); }
    }
    workgroupBarrier();
    ...
    if (threadid.x < RADIX) {
        wg_localHist[threadid.x] = atomicLoad(&hist[threadid.x + info.shift * 32u]) +
